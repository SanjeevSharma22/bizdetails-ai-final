...
export default function App() {
    ...
    if (storedUser) {
      ...
      if (!isTokenExpired(parsed.token)) {
        fetch(`${API}/api/auth/verify`, {
          ...
          .then((data) => {
            const userData = { ...parsed, role: data.role };
            setUser(userData);
  ...
  const handleSignIn = ({ email, name, token, role }) => {
    const newUser = { email, name, token, role };
    setUser(newUser);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex gap-6">
          ...
          >
            ...
            >
              <TabsList
                className={`grid w-full ${user.role === "superadmin" ? "grid-cols-4" : "grid-cols-3"} mb-8`}
              >
              ...
              {user.role === "superadmin" && (
                <TabsContent value="admin" className="space-y-6">
                  <SuperAdminUpload token={user.token} />
                </TabsContent>
              )}
            </Tabs>
          </div>
